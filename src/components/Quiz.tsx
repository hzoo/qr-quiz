...
export function Quiz() {
  ...
  // Setup PartyKit message handler
  useEffect(() => {
    console.log("Setting up PartyKit message handler");
    ...
    return () => {
      console.log("Cleaning up PartyKit message handler");
      // Clean up message handler
      removeMessageHandler(handlePartyKitMessage);
    };
  }, []); // Empty dependency array to ensure handler is only set up once
  
  // Handle messages from PartyKit
  const handlePartyKitMessage = (data: MessageData) => {
    // Always log the incoming message and current question state
    console.log("PartyKit message received:", data, {
      hasCurrentQuestion: !!quizState.value.questions[quizState.value.currentQuestionIndex],
      currentQuestionIndex: quizState.value.currentQuestionIndex,
      totalQuestions: quizState.value.questions.length,
      showResult: quizState.value.showResult
    });
    
    if (data.type === "selection" && data.option) {
      // Always process the scan through handleScan which contains all the logic
  ...
  const handleScan = (value: string) => {
    // Convert to lowercase for command comparisons, but keep original for options
    const originalValue = value;
    const lowerValue = value.toLowerCase();
    
    // Log current quiz state for debugging
    console.log("Scan processing state:", {
      hasCurrentQuestion: !!quizState.value.questions[quizState.value.currentQuestionIndex],
      currentQuestionIndex: quizState.value.currentQuestionIndex,
      questionId: quizState.value.questions[quizState.value.currentQuestionIndex]?.id,
      totalQuestions: quizState.value.questions.length,
      showResult: quizState.value.showResult,
      isLoading: quizState.value.isLoading
    });
    
    // Handle instruction QR code
    if (lowerValue === QR_INSTRUCTIONS.toLowerCase()) {
      setHelpModalOpen(true);
      return;
    }
    
    // Handle close help QR code
    if (lowerValue === QR_COMMANDS.CLOSE_HELP.toLowerCase()) {
      setHelpModalOpen(false);
      return;
    }
    
    console.log("Scan value:", originalValue);
    
    // Handle special commands with prefix "c:"
    if (lowerValue.startsWith(QR_COMMANDS.PREFIX.toLowerCase())) {
      // Handle specific commands
      if (lowerValue === QR_COMMANDS.SHOW_ANSWERS.toLowerCase()) {
        setShowingAnswers(true);
        return;
      }
      
      if (lowerValue === QR_COMMANDS.HIDE_ANSWERS.toLowerCase()) {
        setShowingAnswers(false);
        return;
      }
      
      if (lowerValue === QR_COMMANDS.RESET.toLowerCase()) {
      ...
      if (lowerValue === QR_COMMANDS.ANSWERS.toLowerCase()) {
        console.log("Processing ANSWERS command - showing results");
        quizState.value = {
          ...quizState.value,
          showResult: true,
        };
    ...
    if (lastAnswer) {
      console.log("Ignoring scan - already showing answer feedback");
      return;
    }
    
    // For answer scans, skip if already showing results
    if (showResult) {
      console.log("Ignoring scan - already showing results screen");
      return;
    }
    
    // Get the current question from state to ensure it's fresh
    const currentQuestionFromState = quizState.value.questions[quizState.value.currentQuestionIndex];
    
    // Answer the current question
    if (currentQuestionFromState) {
      console.log("Processing answer for question:", currentQuestionFromState.id);
      
      // First try to find by exact ID match
      let matchedOption = currentQuestionFromState.options.find(o => o.id === originalValue);
      
      // If no match found, try multiple ways to match by simple code
      if (!matchedOption) {
        // Try to match by the entire value (for direct letter inputs like "A", "B", etc.)
        const directMatch = originalValue.trim().toUpperCase();
        
        // Try to extract the last part after underscore (if any)
        const lastPart = originalValue.split('_').pop() || originalValue;
        // Just get the letter part - usually a single letter like A, B, C, D
        const simpleCode = lastPart.trim().toUpperCase();
        
        console.log("Trying to match by simple code:", simpleCode, "or direct match:", directMatch);
        console.log("Available options:", currentQuestionFromState.options.map(o => ({
          id: o.id, 
          letter: o.id.split('_').pop()?.toUpperCase()
        })));
        
        matchedOption = currentQuestionFromState.options.find(o => {
          // Extract letter part from ID (e.g., "A" from "q0_A")
          const optionLetter = o.id.split('_').pop()?.toUpperCase() || '';
          // Match either by extracted letter or the direct value
          return optionLetter === simpleCode || optionLetter === directMatch;
        });
      }
      ...
      console.warn("Cannot process answer - no current question available");
    }
  };
  ...
  const renderConnectionStatus = () => {
    ...
    return (
      <div className="flex items-center gap-1 bg-[#3d3d47] px-2 py-1 rounded-md ml-2">
        ...
        <span className="text-xs">{connectionStatus.value}</span>
      </div>
    );
  };
